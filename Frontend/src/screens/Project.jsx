...
//  import {initZegoCloud} from './oo.js'
// import zg from './oo.js';
function Project() {
...
const [webContainer,setWebContainer]=useState(null)
    // Handle user selection for adding collaborators
                ...
                <div className="conversation-area pt-14 pb-12 flex-grow flex flex-col h-full relative">
                    <div ref={messageBox} className="message-box p-1 flex-grow flex flex-col gap-1 overflow-auto max-h-full scrollbar-hide">
                        {messages.map((msg, index) => (
                            <div key={index} className={`${msg.sender._id === user._id.toString() && 'ml-auto'} message flex flex-col p-2 bg-slate-50 w-fit rounded-md`}>
                                <small className='opacity-65 text-xs'>{msg.sender.email}</small>
                                <div className='text-sm'>
                                  {msg.sender.email === 'AI' ? (
                                        <p>{JSON.parse(msg.message).text}</p>
                                    ) : (
                                        <p>{msg.message}</p>
                                    )}
                                </div>
...
<footer>
                ...
                <div className={`sidePanel w-80 h-full flex flex-col gap-2 bg-slate-50 absolute transition-all ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'} top-0`}>
    ...
    <div className="files flex">
        {
         ...
         <button className='cursor-pointer p-2 px-4 flex items-center w-fit gap-2 bg-slate-300' onClick={
           async ()=>{
        ...
        try {
            ...
            console.error("Error killing previous process:", error);
        }
    }
                                  ...
                                  {console.log(fileTree[ currentFile ].file.contents)}
                                    <pre
                                        ...
                                        <code
                                            ...
                                            dangerouslySetInnerHTML={{ __html: ('javascript', fileTree[ currentFile ].file.contents)  }}
                                            style={{
                                                whiteSpace: 'pre-wrap',
                                                paddingBottom: '25rem',
